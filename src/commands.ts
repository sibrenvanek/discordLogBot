...
export const commands: Array<Command<RichEmbed>> = [
	{
		...
		command: "cpu",
		f: async _params => embedString(await execute("ps -C java -o %cpu").then(output => `${output.replace("%CPU", "").trimEnd()}%`), "cpu")
	},
	{
		description: "Displays the current memory usage in percentages.",
		command: "memory",
		f: async _params => embedString(await execute("ps -C java -o %mem").then(output => `${output.replace("%MEM", "").trimEnd()}%`), "memory")
	},
	{
		description: "Reboots the machine.",
		parameterDescription: "type: confirm\n- confirm: confirms the rebooting of the Pi",
		command: "reboot",
		f: async params => embedString(
			params.length && params[0] === "confirm"
